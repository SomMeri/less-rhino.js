...
Ruleset.prototype.genBodyCss = function(context, output, tabRuleStr, tabSetStr) {
    ...
    var i, rule, rulesetNodeCnt, charsetRuleNodes = [], ruleNodes = [], rulesetNodes = [], sep;
...
Ruleset.prototype.genPathsCss = function(context, tabSetStr, output) {
    var paths = this.paths, pathCnt,
        pathSubCnt, i, j, sep, path;

    if (!paths) { //FIXME: added this here reevaluate whether we want it like this
        return ;
    }
...
Ruleset.prototype.genCSS = function (context, output) {
    var debugInfo;     // Line number debugging

    context.tabLevel = (context.tabLevel || 0);

    if (!this.root && this.paths) {
        context.tabLevel++;
    }
    ...
    if (!this.root && this.paths) {
        ...
        this.genPathsCss(context, tabSetStr, output);
        output.add((context.compress ? '{' : ' {\n') + tabRuleStr);
    }

    this.genBodyCss(context, output, tabRuleStr, tabSetStr);
    if (!this.root && this.paths) {
        output.add((context.compress ? '}' : '\n' + tabSetStr + '}'));
        context.tabLevel--;
    }
